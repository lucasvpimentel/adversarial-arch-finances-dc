"""
Módulo do Nó Agente Bear (Analista Cético/Conservador).
...
def no_agente_bear(estado: EstadoComite) -> dict:
    """
    Nó do LangGraph responsável pela análise CÉTICA / CONSERVADORA (Bear) do ativo.
    
    Consome 'dados_mercado' e 'dados_estatisticos' para construir a tese de risco.
    Retorna APENAS o dicionário com a chave 'argumento_bear'.
    """
    ...
    caminho_prompt = os.path.join("prompts", "agente_bear.md")
    ...
    mensagem_usuario = HumanMessage(
        content=(
            ...
            f"'''\n{dados_estatisticos}\n'''\n\n"
            f"Com base unicamente nessas informações e dados concretos, apresente a sua tese CÉTICA e CONSERVADORA (Bear) "
            f"defendendo os riscos e por que NÃO vale a pena COMPRAR este ativo agora."
        )
    )
    ...
    return {"argumento_bear": resposta.content}
